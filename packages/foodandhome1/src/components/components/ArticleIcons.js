...
const ArticleIcons = ({ showCamera, cameraLink, showVideo, videoLink, commentsLink, categoryText, categoryLink, showTopRight }) => {
  return (
    <>
      <div style={categoryText === undefined ? { justifyContent: "flex-end" } : {}} className="category_image_play_comments_container_1" >
        {
          (categoryText === undefined)
            ...
            <Link link={(categoryLink !== undefined) ? categoryLink : "#category"}>
              <div className="category_container_1">
                <span>{(categoryText !== undefined) && categoryText.toUpperCase()}</span>
              </div>
            </Link>
        }
        {(showTopRight === true) && (<div className="image_play_comments_container_1">
          {showCamera === false ? "" :
          ...
          <Link link={(commentsLink !== undefined) ? commentsLink : "#comments"}>
